...
import lpCompletaDesktop from "@/assets/case-unip/LP-completa-desktop.png";
...
import { CaseQuotesBlock } from "./sections/shared/CaseBlockSection";
import {
  CaseBody,
  CaseEyebrow,
  CaseHeading,
...
interface UnipCaseContentProps {
  caseData: Case;
}

const BASTIDORES_ICONS = [ClipboardList, LayoutTemplate, Megaphone, LineChart] as const;

const BASTIDORES_TAGS = [
  ["Anúncio", "Página", "Contato", "Atendimento"],
  ["WhatsApp", "Conversão", "Marca local"],
  ["Google Ads", "Intenção", "Mensuração"],
  ["Monitoramento", "Ajustes", "Transparência"],
] as const;

function withUnipAssets(caseData: Case): Case {
  return {
    ...caseData,
    heroImage: heroLpDesktop,
    coverImage: heroLpDesktop,
    gallery: [
      {
        src: lpCompletaDesktop,
...
export function UnipCaseContent({ caseData }: UnipCaseContentProps) {
  const data = withUnipAssets(caseData);
        ...
        <CaseSection id="landing-page" className="overflow-visible py-24 sm:py-32">
          ...
          <div className="mt-14 sm:mt-20">
            <MockupResponsiveShowcase
              ...
              mobileSrc={heroLpMobile}
              mobileAlt="Landing page UNIP Caraguatatuba — versão mobile"
              scrollable
              displayUrl="faculdadelitoral.com.br"
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1fr_auto_1fr] lg:gap-8">
            ...
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-full border border-brand/30 bg-brand/10 shadow-[0_0_40px_-10px_oklch(0.72_0.19_48/0.5)] lg:h-16 lg:w-16">
                <ArrowRight className="h-6 w-6 text-brand lg:rotate-0 rotate-90" />
              </div>
            </motion.div>

            {/* Depois — funil conectado */}
            <motion.div
              initial="hidden"
              whileInView="visible"
              viewport={viewportOnce}
